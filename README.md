# 👋 Hi, I'm YoLab

###  YoLab 工作室 | 專為中小企業打造客製化解決方案

我是一位專注於 **AI 視覺檢測**、**自動化系統** 與 **聊天機器人** 開發的軟體工程師。  
擅長將複雜的技術需求轉化為實用、穩定的系統，協助企業提升效率與競爭力。

---

## 🛠 技術棧

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![Java](https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=for-the-badge&logo=cplusplus&logoColor=white)
![YOLOv8](https://img.shields.io/badge/YOLOv8-00FFFF?style=for-the-badge&logo=yolo&logoColor=black)
![OpenCV](https://img.shields.io/badge/OpenCV-5C3EE8?style=for-the-badge&logo=opencv&logoColor=white)
![LINE](https://img.shields.io/badge/LINE_Bot-00C300?style=for-the-badge&logo=line&logoColor=white)
![Discord](https://img.shields.io/badge/Discord_Bot-5865F2?style=for-the-badge&logo=discord&logoColor=white)
![HTML](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)

---

##  接案服務項目

| 服務類型 | 說明 |
|---------|------|
|  **工業視覺檢測 (AOI)** | YOLOv8 深度學習模型、OK/NG 自動判定、產線整合 |
|  **聊天機器人開發** | LINE Bot / Discord Bot 客製化開發 |
|  **資料爬蟲與自動化** | 網頁爬蟲、自動化腳本、排程系統 |
|  **交易系統開發** | 加密貨幣 / 股票 API 串接、策略回測 |
|  **Python 工具開發** | 客製化腳本、批次處理工具、系統整合 |

---

##  精選專案

###  工業 AI 視覺
| 專案 | 說明 |
|------|------|
| [**CCD_System**](https://github.com/Yakitori197/CCD_System) | 產線 AOI 自動光學檢測系統 v1.2：YOLOv8 推理、OK/NG 判定策略分層、序列埠硬體剔除整合、可擴充工業相機介面 |
| [**Yolov8_TrainSystem**](https://github.com/Yakitori197/Yolov8_TrainSystem) | 完整的 YOLO 訓練系統，支援多攝影機、標註、訓練一條龍 |

###  聊天機器人
| 專案 | 說明 |
|------|------|
| [**Stock_LineBot_Public**](https://github.com/Yakitori197/Stock_LineBot_Public) | 台股美股 LINE 機器人（開源版） |
| [**Discord_StockBot**](https://github.com/Yakitori197/Discord_StockBot) | Discord 股票查詢機器人 |

###  實用工具
| 專案 | 說明 |
|------|------|
| [**BatchExtractor**](https://github.com/Yakitori197/-BatchExtractor-) | 批次壓縮檔解壓工具 |
| [**GitHub_UploadSystem**](https://github.com/Yakitori197/GitHub_UploadSystem) | Python 模組化專案樣板 |

---

## 📫 聯絡我

如果您有專案需求或合作洽詢，歡迎透過以下方式聯繫：

[![Website](https://img.shields.io/badge/🌐_YoLab_官網-blue?style=for-the-badge)](https://yakitori197.github.io/YoLab/)
[![Email](https://img.shields.io/badge/📧_Email-聯絡我-red?style=for-the-badge)](mailto:sweiyo1997@gmail.com)

---

<p align="center">
  <i>「用技術解決問題，讓自動化創造價值」</i>
</p>
